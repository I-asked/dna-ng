...
namespace DNA_NG
{
    public static class PDB2WDB
    {
        ...
        public static void Main(string[] args) {
          if (args.Length < 3)
          {
            Console.Error.WriteLine("Usage: " + Environment.GetCommandLineArgs()[0] + " <pe> <pdb> <out>");
            Environment.Exit(1);
          }

          using (var os = File.Create(args[2]))
          using (var pdb = File.OpenRead(args[1]))
          using (var pe = File.OpenRead(args[0]))
          {
            WriteSequencePointsToFile(pe, pdb, os);
          }
        }
    }
}
